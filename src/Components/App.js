...
const App = () => {
  const [weatherData, setWeatherData] = useState([{ id: '', location: '', temperature: '', feelsLike: '', description: '', icon: '' }])
  ...
  const deleteLocation = (id) => {
    console.log(id)
    axios.delete('/weatherData', { data: { id: id } }).then(data => {
      fetchWeather()
    }).catch(error => {
      console.log(error);
    })
    ...
    <>
      ...
      <div id="search-form">
        <form onSubmit={setLocation}>
          ...
          <select onChange={(event) => { setSearchState(event.target.value) }} id="country" name="country">
      ...
      <Cities weatherData={weatherData} deleteLocation={deleteLocation} />
    </>
  )
}
